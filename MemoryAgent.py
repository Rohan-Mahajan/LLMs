...
# Normalize module names in both datasets for consistent matching
defect_df["Module name"] = defect_df["Module name"].str.strip().str.lower()
test_cases_df["Module name"] = test_cases_df["Module name"].str.strip().str.lower()
...
# Step 2: Fetch Correct Test Cases & Provide Solution
def provide_solution(state: AgentState):
    ...
    module_name = context.metadata["module"].strip().lower()  # Normalize input module name
    solution_text = context.metadata["solution"]

    # Fetch correctly matched test cases
    ...
    test_cases_text = "\n".join(test_cases_list) if test_cases_list else "No test cases found for this module."
